...
($E_\text{Pl} \approx 1.22 \times 10^{19}$ GeV)---and its strength is
characterized by the dimensionless coefficients $\eta_a^{(n)}$.

In phenomenological studies, it is standard to assume that the
modification is dominated by a single correction term of order $n$. The
...
relations within the Standard Model Extension (SME), a comprehensive
Effective Field Theory (EFT) framework that includes all
...
responsible for the violation. Typically, terms with even $n$ are
associated with CPT-even operators, which affect all chiral states
similarly. Conversely, terms with odd $n$ usually stem from CPT-odd
operators, which are sensitive to chirality. This distinction leads to
differing phenomenological behaviors for chiral fermions.

In this work, we focus specifically on the Quantum Electrodynamics (QED)
sector. In the standard theory, the photon and the electron (and
positron) are the only stable particles. However, LIV modifications can
kinematically permit decay processes in vacuum that are otherwise
forbidden, such as photon decay ($\gamma \to e^+ e^-$) or vacuum
Cherenkov radiation ($e \to e \gamma$). We will analyze the constraints
arising from these induced instability windows. We first consider
single-parameter models where only one particle species carries the LIV
modification. Subsequently, in Section III, we extend the analysis to
the two-parameter scenario, exploring the consistency of bounds when
both species are modified simultaneously.
